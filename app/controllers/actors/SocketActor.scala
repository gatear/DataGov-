...
object SocketActor {
  def props(out: ActorRef, dbOps: AsyncRepo) = Props ( new SocketActor (out,dbOps))
}

class SocketActor (out: ActorRef, dbOps: AsyncRepo) extends Actor{
  override def receive: Receive = {
    case location: JsValue => Some(Location (location("longitude").as[String], location("latitude").as[String]))
                                                ...
                                                .map((set) => set.collect{ case Tuple5(t , locDesc, desc, date, id) => Crime(t, locDesc, date, id.toLong)})
